...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_patch_tokens: bool = False):
        ...
        x = torch.cat([cls_token, x], dim=1)  # [B, N+1, width]
        ...
        cls_feat = self.ln_post(x[:, 0, :])  # [B, width]
        patch_feats = self.ln_post(x[:, 1:, :])  # [B, N, width]
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, return_patch_tokens: bool = False):
        if return_patch_tokens:
            if isinstance(self.visual, VisionTransformer):
                return self.visual(image.type(self.dtype), return_patch_tokens=True)
            else:
                raise ValueError("return_patch_tokens=True is only supported for VisionTransformer backbones.")
